...
<div class="wrap">
  ...
  <div id="doneState" class="bigstate" style="display:none">
    ...
    <div class="steps3">
      ...
      <div class="s"><b>2 · Pack and ship</b><p id="doneShip">Any sturdy box works. Drop it at any Post Office or USPS pickup.</p></div>
    ...
    <p style="margin-top:10px" id="trackLink"><a href="../../trade-in/track/index.html" style="color:var(--green);font-weight:700;font-size:14px">Track this order →</a></p>
  </div>
</div>
<script>
...
let payMethod = null;
let shipCarrier = "USPS";
// which carriers each category may ship with (mirrors the server rules)
function carriersFor(cat){{
  if(["Cell Phone","GoPro","Smartwatch"].includes(cat)) return ["USPS","FedEx","UPS"];
  if(["Game Console","Tablet","Macbook","iMac","Mac Mini","Mac Studio","Mac Pro"].includes(cat)) return ["FedEx","UPS"];
  return ["USPS"];
}}
function refreshShip(){{
  const c = cartGet();
  let allowed = c.length ? c.map(i => carriersFor(i.cat)).reduce((a,b)=>a.filter(x=>b.includes(x))) : ["USPS"];
  if(!allowed.length) allowed = ["USPS"];
  const show = allowed.length > 1 && payMethod !== "Cash in store";
...
document.getElementById("pay").addEventListener("click", e=>{{
  ...
  document.querySelector(".trust").innerHTML = cash
    ...
    : "<span>Free prepaid shipping label</span><span>Price locked for 14 days</span><span>Paid within 1 business day of arrival</span>";
  refreshShip();
}});
...
document.getElementById("submit").addEventListener("click", async ()=>{{
  ...
  if(shipCarrier !== "USPS" && payMethod !== "Cash in store")
    document.getElementById("doneShip").textContent = "Any sturdy box works. Print the label from your email and drop it at any " + shipCarrier + " location.";
  if(payMethod === "Cash in store"){{
    ...
    const tl = document.getElementById("trackLink"); if(tl) tl.style.display = "none";
  }}
  cartSave([]); cartBadge();
